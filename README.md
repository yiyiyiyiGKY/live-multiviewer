...
原生版包括 3×3 九路预览、同地址共享解码、监听、聚焦、布局排序、重连、分辨率、音频电平和信号异常提示。每个不同地址拥有独立的接入、解码和重连生命周期；完全相同的地址才共享同一个 OBS 媒体源。监看以画面新鲜度优先，OBS 网络媒体缓冲设为 0，以减少为了连续播放而积压的旧画面；实际效果仍须在目标 Windows 设备验证。配置用 Windows 当前用户加密，保存在 `.runtime/native-settings.dat`；已有六路配置会保留并自动补出三个未配置窗口。首次运行可读取旧 `.runtime/settings.json`，但不覆盖它。原生版当前不显示源帧率/码率，也不直接采集 USB 摄像头或 OBS 场景。
...
HLS 当前优先解决兼容性，已配置客户端缓冲限制并关闭单路卡顿后的目标延迟递增；实际长时间运行效果
