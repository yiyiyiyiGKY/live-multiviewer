...
这是 Windows 六路直播监看程序。包内已包含 OBS 32.2.1 媒体运行库，不需要安装 OBS、Node.js、npm 或 FFmpeg。
...
已在开发电脑上验证六路合成音视频、真实 libobs 解码与 D3D11 绘制、同源复用、单路监听、聚焦/排序、黑场/静止/静音检测、来源替换和资源释放，以及 Windows 加密配置读写。包内 `self-test.cmd` 可在接收方电脑运行同样的本地自检，不访问直播地址。

本包属于试用版，尚未完成不同电脑的兼容性测试、真实六路直播的延迟对照、断网恢复和长时间直播压力测试。用于正式直播前，先在目标电脑与实际六路源上彩排，并保留现有监看方式。未承诺达到 OBS 相同延迟或连续多日稳定运行。
